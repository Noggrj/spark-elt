...
from pyspark.sql.functions import col, when, avg, sum, count, desc, year, month, from_json
...
def processar_dados_kafka(spark, bootstrap_servers='localhost:9092'):
    ...
    # Parsing dos dados JSON do Kafka
    schema_clientes = criar_schema_clientes()
    schema_transacoes = criar_schema_transacoes()

    df_clientes = (
        df_clientes_kafka
        .selectExpr("CAST(value AS STRING)")
        .select(from_json("value", schema_clientes).alias("data"))
        .select("data.*")
    )
...
def processar_dataframes_streaming(spark, df_clientes, df_transacoes):
    """
    Processa os DataFrames de clientes e transações em modo streaming.
    """
    # Categorização por faixa etária
    df_clientes = df_clientes.withColumn(
        "faixa_etaria",
    ...
    # Enriquecimento do DataFrame de transações (com conversão de data para timestamp)
    df_transacoes = df_transacoes \
        .withColumnRenamed("valor", "valor_reais")
    
    # JOIN entre clientes e transações para análises mais ricas
    df_join = df_transacoes.join(df_clientes, "id_cliente")
    ...
    # Top 10 clientes (streaming)
    query_top_clientes = df_join.groupBy("id_cliente", "nome", "estado") \
        .agg(sum("valor_reais").alias("total_cliente")) \
        .orderBy(desc("total_cliente")) \
        .limit(10) \
        .writeStream \
